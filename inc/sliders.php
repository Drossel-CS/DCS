...
function custom_postimage_meta_box_func($post)
{
    ...
    foreach ($meta_keys as $meta_key) {
        ...
        */
        ...
        $post_caption = get_post_meta($post->ID, $meta_key.'_caption', true);
        if($post_caption == '0') $post_caption = '';
        ...
        <div class="custom_postimage_wrapper" id="<?php echo $meta_key; ?>_wrapper" style="<?php echo (($image_meta_val_minus_one != '' || $i == 0) ? 'display:block; margin-bottom:10px;' : 'display:none;'); ?> " >
            ...
            <input name="<?php echo $meta_key; ?>_caption" class="caption_input" type="text" placeholder="Popis" style="display: <?php echo ($image_meta_val != '' ? 'block' : 'none');?>" value="<?php echo $post_caption ?>">
    ...
    function custom_postimage_add_image(key, next, prev){
        ...
        custom_postimage_uploader.on('select', function() {
            ...
            $wrapper.find('.caption_input').show();
    ...
    function custom_postimage_remove_image(key, next, prev){
        ...
        $wrapper.find('.caption_input').val('').hide();
    ...
    if (isset($_POST['custom_postimage_meta_box_nonce']) && wp_verify_nonce($_POST['custom_postimage_meta_box_nonce'], 'custom_postimage_meta_box')) {
        ...
        foreach ($meta_keys as $meta_key) {
            if (isset($_POST[$meta_key]) && intval($_POST[$meta_key]) != '') {
                ...
                update_post_meta($post_id, $meta_key.'_caption', $_POST[$meta_key.'_caption']);
                update_post_meta($post_id, $meta_key.'_link', $_POST[$meta_key.'_link']);
                $i++;
            } else {
                update_post_meta($post_id, $meta_key, '');
                update_post_meta($post_id, $meta_key.'_caption', '');
